...
async function GetAll() : Promise<IProductMaster[]> {
	const product :IProductMaster[] = await prisma.product_Master.findMany({include:{stock:true}});
	return product;
}

async function GetById(id:string):Promise<IProductMaster | null> {
	const product: IProductMaster | null = await prisma.product_Master.findUnique({where:{id:id},include:{stock:true}});
	return product;
}
...
async function Create(product:IProductMaster): Promise<IProductMaster | null> {
	...
	const create:IProductMaster = await prisma.product_Master.create({data:prismaProductData, include:{stock:true}});
	return create;
}

async function UpdateById(product:IProductMaster) :Promise<IProductMaster>{
	const update : IProductMaster = await prisma.product_Master.update({where:{id:product.id},data:{
		name: product.name,
		buying_price: product.buying_price,
		selling_price: product.selling_price,
